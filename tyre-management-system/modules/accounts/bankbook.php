...
$rows = array_values(array_filter($rows, static fn($r) => in_array((string)($r['payment_mode'] ?? ''), ['UPI', 'Bank', 'Bank Transfer', 'Cheque'], true)));
$incoming = 0.0;
$outgoing = 0.0;
foreach ($rows as $r) {
    if ((string)$r['tx_type'] === 'Customer Payment') {
...
<div class="accounts-page">
    <header class="prod-page__head">
        <div>
            <h1 class="prod-page__title">Cash &amp; Bank — Bankbook</h1>
            <p class="prod-page__sub">Bank, UPI, and cheque transactions across receivables, payables, and expenses.</p>
        </div>
    </header>

    <div class="sales-kpis accounts-kpis mb-3">
        <article class="sales-kpi"><span class="sales-kpi__label">Incoming</span><strong class="text-success"><?= e(sales_format_money($incoming)) ?></strong></article>
        <article class="sales-kpi"><span class="sales-kpi__label">Outgoing</span><strong class="text-danger"><?= e(sales_format_money($outgoing)) ?></strong></article>
        <article class="sales-kpi"><span class="sales-kpi__label">Bank balance</span><strong><?= e(sales_format_money($incoming - $outgoing)) ?></strong></article>
    </div>

    <form method="get" class="sales-filter-bar mb-3">
        <input type="hidden" name="page" value="accounts/bankbook">
        <div class="sales-filter-bar__row">
            ...
            <div class="ms-auto"><?= erp_export_toolbar('acc-bankbook-table', 'bankbook') ?></div>
        </div>
    </form>

    <section class="sales-card">
        <div class="sales-table-wrap sales-table-scroll">
            <table class="table table-sm mb-0" id="acc-bankbook-table">
                <thead><tr><th>Transaction ID</th><th>Date</th><th>Type</th><th>Party</th><th>Reference</th><th>Mode</th><th class="text-end">Amount</th><th>Status</th></tr></thead>
                <tbody>
                ...
                <?php endforeach; ?>
                <?php if ($rows === []): ?><tr><td colspan="8" class="sales-empty">No bank transactions in selected period.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </section>
